...
public class InternetServiceController {
    ...
    @GetMapping(value = "/internet/form")
    ...
    @PostMapping(value = "/internet/calculatePay")
    public Map<String, String> internetPaymentCalc(@RequestBody Map<String, String> payload) {
    ...
    public Map<String, String> internetPayment(@RequestBody Map<String, String> payload) {
        ...
        for (IAccount account : loggedInUsers) {
            if (Objects.equals(account.getEmail(), email)) {
                if(paymentController.pay(account, system, amount, name + " Internet")) {
